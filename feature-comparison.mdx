...
| Existing-schema contract adoption | Online read-only diff and safe repository generation | Codegen/packages | External schema tooling |
| Native lint/fix/format | Dialect-aware coded diagnostics, reasoned suppressions, safe fixes, and canonical formatting | External tools | Built-in formatter plus external linting |
