...
| dbt-native state and defer | Forwards `--state`, `--defer`, and dbt selectors while coordinating downstream SQLBuild work | dbt State | N/A |
| Explicit dbt clone and diff | Resolve selected production-shaped relations from a configured git ref for clone or comparison | dbt clone/state workflows | N/A |
