...
Try the integration without setting anything up:
...
This creates the waffle shop project with a `dagster/definitions.py` that includes asset definitions, scenario checks, and a configured resource. The definitions use `prepare_if_dev()` to auto-generate the DAG artifact when Dagster starts.
