...
SQLBuild keeps a low, dbt-like floor for SQL models, then adds what you'd normally reach for other tools for: pre-promotion audit gating, local E2E testing, ingestion, and Python tasks, assets, and checks. Virtual environments are there when you want instant, low-copy branching and promotion - opt-in, never forced on you. It can run alongside an existing dbt project, so you can adopt it incrementally.
