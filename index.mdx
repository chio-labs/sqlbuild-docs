...
- **Keep SQL consistent natively.** `sqb lint`, `sqb fix`, and `sqb format` share SQLBuild's dialect-aware parser and model semantics, without a separate linter installation.
