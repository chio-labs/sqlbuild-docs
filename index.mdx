---
title: "Introduction"
description: "A SQL-first data framework: transformation, ingestion, Python tasks and assets, and testing in one tool."
---

SQLBuild is a SQL-first data framework that handles transformation, ingestion, Python tasks and assets, and testing in one tool.

It keeps a low, dbt-like floor for SQL models while adding compile-time validation, pre-promotion audit gating, and full local E2E testing. It can also run alongside an existing dbt project, so you can adopt it incrementally.
...
- **Advanced virtual environments:** Optional state-backed workflows for low-copy branching, promotion, and rollback when your team is ready to operate a shared state store. See [Virtual Environments](/concepts/advanced/virtual-environments).
