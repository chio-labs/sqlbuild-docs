...
State is plain append-only rows in your own warehouse (`_sqlbuild_fingerprints`, `_sqlbuild_source_freshness`, `_sqlbuild_node_results`) - no external state database, no manifest files, no state machine that can corrupt. This change-aware behavior applies to SQLBuild-owned work. When you point SQLBuild at an [existing dbt project](/concepts/dbt-compatibility/overview), dbt selection and execution remain dbt-native, including `--state` and `--defer`.
