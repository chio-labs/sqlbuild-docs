...
Every build fingerprints models, seeds, functions, and Python nodes, tracks source freshness, and skips anything that hasn't changed, including audits that already passed for the same version. It can run alongside an existing dbt project, and adds ingestion, Python nodes, and opt-in virtual environments for more advanced use cases.
