...
In direct mode, the janitor never drops a live relation directly. Each stale relation goes through two stages:
...
When `retention_days` is not set, direct mode uses `14` and virtual-environment mode uses `30`. See [Project Configuration](/concepts/project-configuration#janitor) for every janitor setting.
...
In virtual-environment mode, the janitor also cleans up expired environments, old checkpoints and unreferenced physical versions. See [Virtual environments: janitor](/concepts/virtual-environments/janitor). Direct-mode archiving doesn't apply there. In both modes, relations whose names differ only by case from another relation in the same schema are reported and never dropped, and exclude patterns match regardless of case.

## Safety

The janitor shows every planned archive and delete, with exact names, ages and expiry, then asks you to type an exact confirmation phrase. Use `--auto-approve` only in automation, or when you're certain.
