...
| Flag | Description |
|------|-------------|
| `--select`, `-s` | Select scenarios (merged with positional selectors) |
...
Scenarios can be selected positionally or with `--select`. Both forms are merged. `--exclude` removes scenarios from the selection. Without any selectors, all discovered scenarios run.
...
| `--select` flag | `sqb scenario test --select daily_revenue_minimal` |
| `--exclude` flag | `sqb scenario test revenue --exclude daily_revenue_multi_order` |

Positional and `--select` selectors are merged, mixed selector types are supported, and the result is de-duplicated by scenario name.
