...
- **Column lineage**: Traces which source columns flow into each output column, including transform classification (direct, cast, expression, star)
...
Column lineage results are included in the JSON compile report under each model's `lineage` field.
