...
custom rules are trusted Python code.
...
<Note>
In SQLBuild 0.54.1, `--exclude` is applied only when `--select` is also provided. To evaluate all
models except one, start with an explicit broad selector such as `--select path:models`.
</Note>
