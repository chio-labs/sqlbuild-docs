...
If execution is interrupted or a query raises an infrastructure error, SQLBuild stops scheduling
new audits, drains queries that have already started, and then closes every worker connection.
Generic adapters cannot guarantee immediate cancellation of a query already running in the
warehouse.
