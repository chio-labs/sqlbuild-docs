...
@kata(
    ...
    message="order models must declare a customer_id column",
    remediation="Declare customer_id in the MODEL columns contract.",
)
def customer_identity(*, model, ctx: RuleContext) -> list[KataFault]:
    if any(column.name == "customer_id" for column in ctx.declared_columns):
        return []
...
REQUIRED_DOMAIN = RuleOption.string(
    ...
    default="commerce",
    description="Domain that owns order models",
)
...
@kata(
    ...
    message="order models must belong to the configured domain",
...
```toml
[kata.rule_options.XSQBKO002]
required_domain = "commerce"
```
...
from kata.rules.orders import customer_identity


def test_missing_customer_identity_faults() -> None:
    result = evaluate_rule(
        rule=customer_identity,
        test_case=RuleCase(
            description="missing customer identity faults",
            source=(
