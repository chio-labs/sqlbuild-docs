...
| `retention_days` | `14` in direct mode, `30` in virtual-environment mode | How many days a relation must be stale before it is archived (direct mode) or cleaned up (virtual-environment mode) |
| `archive_retention_days` | `14` | Direct mode only: how many days an archive is kept before it is deleted. `0` deletes it in the same run. |
...
| `max_checkpoints` | `20` | Virtual-environment mode: how many checkpoints to keep |

See [janitor](/cli/janitor) for the archive and delete lifecycle.

## Scenario

Configuration for scenario run isolation, snapshot capture safety limits, and local type overrides:

```toml
[scenario]
run_namespace = "orders-job-123" # Optional; omit to preserve existing scenario relation names
...
| `scenario.run_namespace` | unset | Optional 1–128-character namespace using ASCII letters, digits, `-`, `_`, or `.`. CLI `--scenario-namespace` overrides `SQLBUILD_SCENARIO_NAMESPACE`, then local config, then project config. |
