...
| `changes_only` | `false` | Enable native SQLBuild [change-aware pruning](/concepts/planning#changes-only-mode) for `plan` and `build` without passing `--changes-only` each run. This setting does not prune dbt resources or alter ordinary `sqb dbt` orchestration. Can also be set per target. |
...
| `vars` | Default dbt variables. Local and CLI variables override matching keys. |
| `production_ref.git_ref` | Production-shaped git branch or tag compiled only by explicit `sqb dbt clone` and `sqb dbt diff`. Required for those commands. |
| `production_ref.generate_schema_name_override` | Relative path to the `generate_schema_name` macro injected when compiling the production ref. Required for clone/diff. |
| `production_ref.refresh` | Refresh the cached production checkout before compiling. Defaults to `true`. |
| `production_ref.git_timeout_seconds` | Timeout for production-ref git operations. Defaults to `30`. |

Paths can be absolute or relative to the SQLBuild project root. Local `[dbt]` configuration supports only `target` and `vars`. Ordinary `sqb dbt plan/run/build/test` commands do not use `production_ref`; they compile only the current project. See [Using SQLBuild with dbt](/concepts/dbt-compatibility/overview) for setup and usage details.

The removed `[dbt].replay_on_change` and `[dbt].defer_clone_from` keys produce a configuration error. They are not compatibility aliases.
