...
Kata is opt-in. Prefix selectors activate matching default-enabled rules; exact codes also activate
opt-in rules. See [Kata SQL Architecture Checks](/concepts/kata) for the complete rule,
configuration, and suppression reference.
