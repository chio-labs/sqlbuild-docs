...
A Rule checks authored SQL or the compiled project without querying warehouse data. For example, a
Rule can ask:
...
Built-in Rules cover reusable requirements such as SQL safety and dependency boundaries. Custom
Rules encode requirements specific to your project. Both use facts SQLBuild already has about SQL,
models, paths, configuration, declarations, columns, contracts, tests, audits, and the resolved
graph.
...
First, select an exact Rule or a family of Rules in `sqlbuild_project.toml`. This example enables the
complete built-in SQL family:
...
The selected SQL Rules report the unordered `LIMIT` as a finding and compilation fails before
SQLBuild completes artifacts or opens a warehouse connection. Each finding identifies the Rule,
source location, problem, and remediation. In this case, add deterministic ordering and compile
again:
...
Use focused commands when inspecting or adopting one Rule:
