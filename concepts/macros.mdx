...
Create Python files under `macros/` in your project. Every public ordinary function defined by a macro module becomes a callable macro:
...
Macro output cannot contain macro calls. Expansion is single-pass: if a macro returns SQL containing `@another_macro()`, SQLBuild raises an error.
