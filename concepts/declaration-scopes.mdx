...
Model-private constants and enums declared with an underscore name in `MODEL()` are different: they belong only to that model. They are available in the owner's query and inline SQL hooks, but not to descendants, tests, scenarios, or any other resource.
