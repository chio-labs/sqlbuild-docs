...
This works on a plain dbt project with no changes to your dbt files. SQLBuild reads the dbt manifest, compares it against state it stores in your warehouse, and decides per model whether dbt needs to run it.
...
When a changed dbt model is rerun, you can control how much data is rebuilt with the `replay_on_change` policy in the `[dbt]` config block:
...
This applies the rebuild window when a changed incremental dbt model is rerun, mirroring the [`replay_on_change`](/concepts/planning#replay-on-change) policy SQLBuild uses for its own incremental models.
