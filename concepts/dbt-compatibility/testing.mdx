...
1. **dbt tests run first** from the exact data/unit test nodes resolved during planning.
...
```bash
# All selected unit tests, without data tests or SQLBuild audits
sqb dbt test --select test_type:unit

# Unit tests associated with one dbt model
sqb dbt test --select "fact_orders,test_type:unit"
```

The planned test IDs are reused for execution, progress, and the final summary. SQLBuild tests may target dbt models directly with `__expected__<model>`, including dbt model names that contain double underscores; exact model names are resolved before package-qualified fixture encoding.
