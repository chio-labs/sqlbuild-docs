...
Selectors that SQLBuild does not recognize (like `state:modified`, `package:stripe`, `source:stripe.charges`) are passed to `dbt ls` to resolve. State selectors also need a dbt state artifact:
...
# Build SQLBuild models downstream of dbt-native modified models
sqb dbt build --state path/to/state --defer --select state:modified+
...
1. **dbt test runs first** - planning resolves the user's selectors to exact dbt `test` and `unit_test` nodes, and execution reuses that test set
...
Ordinary interop commands do not apply SQLBuild-owned change detection to dbt resources. Use dbt-native `--state` and selectors such as `state:modified` when you want dbt state semantics.
