...
You can use `sqb dbt` with no SQLBuild models at all. This page is the optional next step: write SQLBuild models, tests, audits, and scenarios downstream of your dbt project without leaving it.
...
This resolves to the qualified warehouse table name from the dbt manifest (e.g. `analytics.fact_orders`). The dbt model becomes an upstream dependency in the combined graph, so selection, execution ordering, and failure blocking work across the boundary. SQLBuild does not fingerprint the dbt model or infer that it changed; use dbt-native state selectors when you need change-based dbt selection.
