...
- **`@` syntax** is for SQL - macros, variables, and context in authored SQL text (model queries, hooks, tests, audits)
- **`${...}` syntax** is for config - context templates in TOML/YAML config values and inline source expressions

These are separate layers resolved at different compilation stages. They never mix: `@` syntax does not appear in config files, and `${...}` syntax does not appear in SQL.

## Syntax reference

| Syntax | Where | Resolved |
|--------|-------|----------|
| `@macro(args)` | Model SQL, hooks, tests, audits | Compile time - expands to macro return value |
| `@@name` | Model SQL, hooks, tests, audits | Compile time - project variable substitution |
| `@@ENV:NAME` | Model SQL, hooks, tests, audits | Compile time - environment variable |
...
| `${CTX:...}` | TOML/YAML config, inline source expressions | Config compilation |
| `${ENV:...}` | TOML/YAML config | Config compilation |
...
1. **Config templates** (`${CTX:...}`, `${ENV:...}`) in TOML/YAML config and inline source expressions are resolved during config compilation
