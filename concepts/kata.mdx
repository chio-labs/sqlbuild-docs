...
This is an example policy, not a universal preset. Select only conventions that the repository is
ready to enforce.

<Note>
Kata evaluates no rules until `[kata].select` resolves at least one rule. Prefix selectors activate
only matching rules that are enabled by default. Exact rule codes also activate opt-in rules. In
SQLBuild 0.54.1, `select = ["SQBK"]` activates `SQBKR401` only; it does not enable every built-in
rule.
</Note>
...
All built-ins are opt-in except `SQBKR401`.
...
| `SQBKR401` | Models declare `contract enforced` (default-enabled) |
...
| Code | Check |
|------|-------|
| `SQBKH001` | Enum comparisons use declared enum members |
