...
This activates the complete built-in policy. Select a narrower family prefix or exact codes when a
repository is not ready to enforce every convention.

<Note>
Kata evaluates no rules until `[kata].select` resolves at least one rule. Prefix selectors activate
matching default-enabled rules. All built-ins are default-enabled within this opt-in policy, while
custom rules are default-off unless their definition says otherwise. Exact rule codes activate
individual rules regardless of their default.
</Note>
...
All built-ins are default-enabled when selected by a matching prefix. Kata itself remains opt-in.
