...
## Cloning upstream inputs from production
...
This lets you build and test a subgraph in dev against real production inputs without rebuilding everything upstream of it. Cloned inputs respect the same `reuse_hard_copy` and zero-copy clone behavior as [reuse from production](#reuse-from-production), and appear under `Dependency baseline` in the plan output.
