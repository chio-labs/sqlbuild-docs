...
- **No catch-up.** You get production's current data as the input, exactly as it is. SQLBuild does not run an incremental delta on a dependency baseline, even if the upstream is an incremental model.
- **Not fingerprinted.** A dependency baseline is materialized only to feed the build, so SQLBuild does not write a fingerprint for it. This is deliberate: the upstream was never actually built or validated in this target, so recording it as current would let change detection wrongly skip it later. The next time you select that model, it is planned from scratch as if the clone had never happened.

This lets you build and test a subgraph in dev against real production inputs without rebuilding everything upstream of it. Dependency baselines respect the same `reuse_hard_copy` and zero-copy clone behavior as [reuse from production](#reuse-from-production).
