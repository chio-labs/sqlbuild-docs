...
description: "How selection interacts with change detection, and the stale warnings that stop silent partial rebuilds."
---
...
For a scoped native run like `sqb build --select agg_daily_revenue`, the plan shows only the selected resources, not their passive upstream closure. SQLBuild still walks upstreams internally for change propagation, but those passive upstreams are not listed as plan entries or counted in the header.
...
```bash
sqb plan --select agg_daily_revenue
```
...
Current (1)
  agg_daily_revenue  no change

Warnings (1)
  - selected model 'agg_daily_revenue' is stale: upstream stg_orders changed
...
Plan ready (8 selected resources)
  ...
  Model plan
    Checksum changed (1)
      stg_orders         query changed
    Upstream changed (4)
      agg_daily_revenue  upstream changed
      fct_orders         upstream changed
      int_order_payments upstream changed
      stg_order_statuses upstream changed
```
...
<Note>
This page describes native SQLBuild planning. Ordinary `sqb dbt` commands do not classify dbt resources with SQLBuild fingerprints or emit SQLBuild-owned dbt staleness warnings.
</Note>
