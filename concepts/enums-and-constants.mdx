...
member's stored value is lowercase. Member lookup is case-sensitive, so `.DELIVERY` is valid for
`ACTIVE "active"` and `.win` is not. Duplicate declaration names, duplicate member names, invalid
