...
1. **dbt runs first** - the primary `dbt run/build` command executes with the user's original selectors
2. **Supplemental dbt run** - if SQLBuild models require dbt models that weren't in the primary selection, a second dbt run targets those specific models
3. **SQLBuild runs second** - selected SQLBuild models execute against the now-built dbt tables
...
SQLBuild unit tests work normally with dbt interop models. Mock the SQLBuild model as usual:
...
The dbt dependency is resolved at compile time from the manifest, so tests run without a warehouse connection.
