...
<Note>
Virtual environments are in alpha. The core workflow (build, promote, rollback, reconcile) is functional and tested across supported adapters. The API and CLI surface may evolve based on feedback. We recommend using virtual environments in development and staging before adopting them for production workloads.
</Note>
