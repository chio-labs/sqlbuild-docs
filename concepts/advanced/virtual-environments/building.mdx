...
def prepare_version(ctx: VersionPrepareContext) -> None:
    """Prepare the new physical target from the prior version."""
    ctx.execute_sql(f"CREATE TABLE {ctx.target} AS SELECT * FROM {ctx.prior_relation}")

def materialize(ctx: MaterializationContext) -> MaterializationResult:
    """Run the custom materialization logic against the prepared target."""
    ...
```
...
`VersionPrepareContext` provides `prior_relation` (the source physical version), `target` (the new physical target), `adapter`, `connection`, `execute_sql()`, `config`, and `vars`.

Custom materializations in virtual mode must write only to `ctx.target`. Side-effect writes to other relations are not tracked by virtual state, not cleaned up by janitor, and not restored by rollback.
