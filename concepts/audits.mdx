...
The `@column` and `@model` parameters are substituted at compile time based on where the audit is attached. Custom parameters like `@expression` and `@'values'` are passed from the audit declaration.

### Attaching generic audits

Attach audits to models in the `MODEL()` header using the `columns` and `audits` fields:
